...
Local web app for ingesting TikTok URLs, X/Twitter URLs, Reddit URLs, YouTube URLs, and local video files, then preparing media for review-first social publishing workflows.

## Features

- Download video from supported TikTok, X/Twitter, Reddit, and YouTube URLs with `yt-dlp`
...
- Optionally create highlight clips under `clips/` using transcript scoring and scene change detection
- Highlight captions can run in safe mode to replace explicit transcript snippets with neutral posting text
...
## Production Posting Strategy

Use these paths in this order:

- `YouTube Shorts` native upload first
- `Instagram Reels` native upload second
- `TikTok` manual posting first, then official API only after audit and policy review
- local file or owned media URL as the preferred posting source

Do not treat third-party scraper flows as a production contract. If URL ingest is unstable, switch to local file upload and continue the pipeline from there.
...
- `SOCIALAUTOPOST_WHISPER_DEVICE="cuda"` uses the GPU for `faster-whisper` when the local install supports CUDA
- on older NVIDIA cards or partial CUDA installs, prefer `SOCIALAUTOPOST_WHISPER_COMPUTE_TYPE="int8"`
- `SOCIALAUTOPOST_FFMPEG_VIDEO_ENCODER="h264_nvenc"` uses NVIDIA NVENC for normalize/highlight exports
- leave these vars unset to keep CPU defaults
- `SOCIALAUTOPOST_SAFE_CAPTIONS` defaults to `true`; set it to `false` if you want raw transcript snippets in clip captions
...
1. `Shorts` native
2. `Reels` native
3. `TikTok` manual-first until official Content Posting API approval is complete

The generic token + endpoint webhook path is for internal integration and testing. It should not be treated as the long-term production path for TikTok or Reels.

For generic live mode, configure per-platform token + endpoint env vars:
...
### Instagram Reels Native Adapter

Recommended next production adapter:

- native container + publish flow against the Instagram professional-account API
- use media files generated by `exports/clip_xx/final.mp4`
- keep the generic webhook path only as a staging bridge until the native adapter exists

### TikTok Posting Policy

Use TikTok live autopost only when all are true:

- the account flow uses TikTok's official posting APIs
- the app has passed the required audit/review for the intended visibility mode
- the export screen honors creator-specific limits and privacy options

Until then, treat TikTok as `manual posting first`.
